...
GBNRdtSender::GBNRdtSender() : expectSeqNumSend(0), WaitingState(false), wndleft(0), wndright(0) {	//初始化wndright为-1，则可以取消特判，即发送第一个包裹之前直接加至0；
...
bool GBNRdtSender::send(const Message& message) {
	...
	pUtils->printPacket("GBN发送方发送报文", SendWnd[wndright]);
...
void GBNRdtSender::receive(const Packet& ackPkt) {
	if (wndleft == wndright) {	//发送窗口为空则直接返回
		cout << "GBN发送方发送窗口为空，无需接收确认报文" << endl;
		return;
	}
	...
	if ((checksum == ackPkt.checksum) && (checkrange1 || checkrange2)) {	//执行操作的条件是checksum无误且序列号是在确认窗口里
		...
		while(SendWnd[wndleft].seqnum != ackPkt.acknum){
			pUtils->printPacket("GBN发送方正确收到确认，正在累积确认中", SendWnd[wndleft]);
			wndleft = (wndleft + 1) % GBNMOD;
		}
		pUtils->printPacket("GBN发送方正确收到确认，正在累积确认中", SendWnd[wndleft]);	//把相等的也确认
		wndleft = (wndleft + 1) % GBNMOD;
		...
		if (checksum != ackPkt.checksum) {
			cout << "GBN发送方收到的确认包校验有误" << endl;
		}
		else { cout << "GBN发送方收到的确认包序号有误" << endl; }	//GBN如果收到了过期的ack或checksum有误的ack怎么处理？直接超时。
	}
	printwnd();	//打印当前滑动窗口
...
void GBNRdtSender::timeoutHandler(int seqNum) {
	...
	for (int i = wndleft; i != wndright; ) {
		pUtils->printPacket("GBN发送方定时器时间到，重发超时报文开始的所有报文", this->SendWnd[i]);
